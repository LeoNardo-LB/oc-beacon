...
- [~] **#122 状态性能与 AI Agent 功能批次（D2-25 AutoApprover 已接线 e3cde191；D2-15/D2-19 待做）** `perf` `sse` `ai-agent`
  ...
  - **2026-08-19 D2-15 完成（a7f07039）**：applyTransition 仅时间戳变化（结构相等）且增量 <1s 的事件短路——跳过整表 O(n) Map 拷贝（返回原引用 → StateFlow equality O(1) 短路）+ history 追加（无转移可记，语义更准）；副作用/日志不受影响。lastEventAt 以 ≤1s 粒度刷新，全部消费阈值（15s/3min/24h/60s）无损。根因：lastEventAt 每事件必变 → 状态恒"不等"，但稳定活动态下差异仅剩时间戳。测试：28 测试全绿（含压测改交替真实转移的语义修正 + 2 新回归）；E2E：300 词流式生成短路生效 + UI 逐字正常 + Idle 收尾 + 40s 窗口 0 次 L2 误判。**D2-19（SSE Last-Event-ID 续传）仍待做**
