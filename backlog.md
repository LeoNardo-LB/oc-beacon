...
- [~] **主对话抽屉屏占比高度统一——最小/最大高度统一为 75%** `ui`
  ...
  - **真机测试 runbook（本批次打通，后续复用）**：① MIUI 需开「USB 安装」且**装时屏幕必须解锁常亮**否则 INSTALL_FAILED_USER_RESTRICTED（adb -s e69a99d8 shell svc power stayon usb 可保常亮）② 服务器打通用 adb reverse tcp:4199 tcp:4199（设备 127.0.0.1:4199 → 宿主机）③ 一键配置服务器走 debug 构建 intent：am start -n dev.leonardo.ocbeacon.dev/dev.leonardo.ocbeacon.MainActivity --es debug_url http://127.0.0.1:4199 --es debug_username opencode --es debug_password <pwd>（仅 BuildConfig.DEBUG 生效；dev-release 本地无 keystore 回退 debug 签名且非 debuggable 不可降级覆盖）④ 本地 keystore 已失（仅 CI Secrets 存留）——本地构建恒为 debug 签名，与 CI release 包互不覆盖，切换需卸载重装
  - ⚠️ 待用户验收：观感（固定高度后空内容抽屉底部留白是否符合预期）——测试构建已在真机可直接体验
- [ ] **新增 P3：SystemPromptDialog + extractSystemPrompt 疑似死代码（2026-08-20 抽屉统一批次顺带发现）** `refactor`
  - 现象：全库 grep 仅定义处自引用，无任何调用方（聊天界面无系统提示词入口）；本次已顺手将其高度改齐 75%（若复活即合规），死代码清理另行处理
  - 工时：~0.5h | 难度：低 | 涉及：ui/screens/chat/dialog/SystemPromptDialog.kt | 优先级：P3
