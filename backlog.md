...
- [~] **#32 归档压缩（二期：热/冷分层 + 整桶 zstd + TLRU 淘汰）** `data` `room` `cache`
  ...
  - **2026-08-09 二期启动**：一期（#30）模拟器验证已全覆盖（14 项 + 补充 3 项，仅剩真机最终确认，用户暂不在附近）；用户决策"先开发二期，注意提交隔离"→ **开发分支 `feature/archive-compression`**（基于 master 954e3c89，未合回）；zstd-jni 最新稳定版 **1.5.7-10**（2026-06-14，Maven Central，Android AAR 支持 API 21+）；二期完成并经验证后合回 master
  - **2026-08-09 二期代码完成（待人工验证）**：6 任务全链路开发完毕——Task1 zstd-jni 集成 + ZstdCodec（1.5.7-10）；Task2 ArchiveBucket 表/Entity/DAO + v3 Migration；Task3 MessageStore 归档编排（prune 前整桶归档 + 桶保护上限 + [archive]/[dearchive] 日志）；Task4 MessageCacheRepository 归档读取接口 + loadArchivedRange/hasArchivedMessages 实现；Task5 翻页归档优先（LoadOlderResult + ARCHIVE/NETWORK 来源，归档只进内存）；Task6 clearSession 级联清理（清热表+归档）。全量单测 **1338 通过/0 失败**（含归档相关 15 用例：MessageStoreTest + Migration + DAO 测试），编译 ✅（main+androidTest）。⚠️ 真机验证待用户：注入 >1100 条消息触发归档 → 查 archive_buckets；断网向上滚动 → logcat 查 [paging]/[dearchive]，UI 显示历史
