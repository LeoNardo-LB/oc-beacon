...
- [~] **提问卡片 M3 原生化改造（消除"外来物"拼盘感）** `ui`
  ...
  - **2026-08-17 E2E 三轮实测（模拟器独占，真实 V2 服务器）**：
    ...
    - **样式终态（第四轮用户决策 f191a876）**：选择指示 = 右侧 ✔（选中才显示，未选中无控件）——用户看过真机截图后弃用左侧常驻 Radio/Checkbox（M3 选择列表标准模式）；ListItem 行结构保留。E2E 像素级验证 6/6：左侧零控件残留（像素扫描）、✔ 落 trailing 槽位（x902-936@420dpi≈18dp）、淡染生效、FATAL=0（截图 /tmp/e2e5/）
  ...
  - ⚠️ 人工验收待用户：整体观感（维度 5 视觉目测，截图在 /tmp/e2e3/ /tmp/e2e4/ /tmp/e2e5/）
