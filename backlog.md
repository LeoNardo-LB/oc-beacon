...
- [ ] **#239 流式残余「细微卡顿」观察——形态定性后定向** `perf` `sse`
  - 背景（2026-08-27 用户验收）：混搭矩阵修复（0775582d）+ devRelease 后用户反馈「好很多了，还有很细微的卡顿」——主线程已健康（atrace doFrame p99=8.41ms 在 120Hz 预算内，95%+ 帧间隔正常）
  - 嫌疑清单（journal 2026-08-27-streaming-jank-matrix-fix.md）：①服务器 14s token 爆发节奏（已实证 flush gap p50=14157ms，换快模型可分离）②补偿/fling 竞态零位移帧（~5 次/会话，根治需动铁律③ compensation-yields-to-fling 专项）③48ms flush 固有步进（20Hz 更新 vs 120Hz 显示，设计权衡）
  - 定向路径：等用户描述形态（蹦字步进→①/fling 顿挫→②）后决定是否升级专项
