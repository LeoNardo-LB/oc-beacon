...
- [ ] **#299 DSH 会话进场分页加载 ~1 页/s——进场链路串行页管线提速** `dsh` `perf`
  ...
  - **2026-09-03 巨型载体实测（journal 307 §299-245，用户点拨从 DB 定位）**：15 万条桶/9.3 万事件巨型会话（session-a6c4）冷进场 **26s/46 页 history/吞吐 ~360 msg/s**、加载指示正常消失、渲染正常——体量线性无退化（412 条 3.7s ↔ 15 万条 26s），管线健康；26s 超大冷进场体感是否可接受留用户日常判断
...
- [ ] **#245 巨型消息区下滑翻旧偶发「拖不动」——方向不对称滚动死帧** `ui` `sse`
  ...
  - **2026-09-03 第三仪器证伪（journal 307 §299-245）**：巨型会话载体（15 万条）上 `input motionevent` 逐事件慢拖（方向正确 300px/30 步）**零滚动**，同方向 `input swipe` 立即滚——逐事件注入不构成被消费的拖拽语义；三种仪器（swipe 批处理/motionevent 不认领/无 root sendevent）全部失效，注入层彻底无解，维持用户真人样本唯一路径
