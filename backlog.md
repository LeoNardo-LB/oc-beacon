...
- [ ] **#245 巨型消息区下滑翻旧偶发「拖不动」——方向不对称滚动死帧** `ui` `sse`
  - 手势阶梯实验（e234g-REPORT）：数屏长单项区域下滑 120/300ms ×8 全部帧字节级静止（UP/DOWN 方向不对称——上滑正常；短消息区正常；moveCount 完整送达排除丢事件/注入假象）；疑似 isAtBottom/autoScroll/延迟揭示补偿对翻旧方向的钳制（#239 竞态修复族邻近机制）
  - 待办：独立诊断会话定位钳制源（AGENTS.md SSE 滚动铁律域）；真人影响面需 V6 人工验证补证（长消息手指下滑是否偶发拖不动）；与 #234 改动无直接关联（本轮未触滚动机制）
