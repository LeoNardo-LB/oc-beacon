...
- [ ] **#245 巨型消息区下滑翻旧偶发「拖不动」——方向不对称滚动死帧** `ui` `sse`
  - 手势阶梯实验（e234g-REPORT）：数屏长单项区域下滑 120/300ms ×8 全部帧字节级静止（UP/DOWN 方向不对称——上滑正常；短消息区正常；moveCount 完整送达排除丢事件）；四轮复测 T2 一度判全档失效后自我更正为测量假象嫌疑（tap 正常+大幅 swipe 可动）——#245 维持「嫌疑 + 未确证」，终裁以独立手势标定为准
