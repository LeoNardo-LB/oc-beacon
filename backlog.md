...
- [ ] **#239 流式残余「细微卡顿」观察——剩服务器节奏单嫌疑** `perf` `sse`
  - 背景（2026-08-27 用户验收）：混搭矩阵修复（0775582d）+ devRelease 后「好很多了，还有很细微的卡顿」；48ms flush 用户确认无感（排除）
  - ~~②补偿/fling 竞态零位移帧~~ **已根治**（744967f4 + 二审纠偏 e7b23f99）：滚动保持（holdReveal）——滚动中不注入不全揭示（首版全揭示 = 内容被顶起回归，勘误见 journal）；A/B survived 24 → 0 保持，视觉验收待用户
  - 剩①服务器 14s token 爆发节奏（flush gap p50=14157ms 实证，换快模型可分离）——用户日常使用中如仍见「蹦字步进」即此因，app 侧无事
