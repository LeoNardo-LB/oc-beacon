...
- [ ] **#271 本地全量保留策略：移除冷存桶 LRU 淘汰/取消 reasoning 截断/首开 drain 全量历史** `data`
  ...
  - 方案（2026-08-30 用户裁决）：移除桶自动淘汰（占用统计+手动清理兜底）；reasoning 取消截断；工具输出维持 500 预览（服务器重拉兜底）；首开自动后台 drain 全量历史；同步指示=会话内顶部进度条+列表摘要行「同步中」芯片（仅同步中显示），详情与触发在长按菜单
