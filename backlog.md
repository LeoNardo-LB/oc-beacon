...
- [~] **#231 「还是叠在一起」二审：跨 item 越界绘制构造性防御 + 视觉误读勘误** `ui`
  - 用户再报；交叉取证定音：真重叠存档属 #230 前炸弹数据期（清源后未再捕获）；当晚多个「OVERLAP」判定为低分辨率视觉模型把 Markdown 行内代码误读为叠层（全分辨率人读式复核全部翻案）；语义树 bounds 交集恒 0
  - 修复：非流式 Turn/Chunk/UserChunk item 补 clipToBounds——异步增长重排窗口的越界绘制转为暂时裁掉，跨 item 叠加构造性不可能；stress fling + 稳定态复核无重叠，1956 单测绿
  - → `docs/journal/2026-08-25-session-list-wipe.md` §#231

- [~] **#230 消息重叠（叠在一起）：服务器空 part 全通道根治 + #228 清扫误删勘误** `data` `ui` `v2`
  - 用户报（先于当日改动存在）；RESIZE +914px 渐进测量实锤 + 服务器 content 本身携带 209 空 reasoning（#223 结论对该会话不成立）→ turn 渲染 211 part 渐进布局，增长窗口文本压邻居
  - 修复五通道：V2/V1 REST 映射源头滤空（ordinal 保契约）+ 空 started 零注册 + delta 重建按 id 契约判型（连带真 bug）+ flush blank 守卫 + #228 清扫谓词勘误（`text IS NULL` 误删全部 reasoning 缓存——Reasoning 行 text 列恒 NULL 属 #79 落库设计）
  - 验证：true empties=0 跨重启保持、健康 reasoning 61 条保留、1956 单测绿、录屏无重叠；残留单次 RESIZE（reasoning 展开/异步首测）待用户观感裁决
  - → `docs/journal/2026-08-25-session-list-wipe.md` §#230
