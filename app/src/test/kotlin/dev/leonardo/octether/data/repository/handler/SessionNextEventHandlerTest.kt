package dev.leonardo.octether.data.repository.handler

import dev.leonardo.octether.domain.model.SessionNextEvent
import dev.leonardo.octether.domain.model.SessionNextEvent.ToolOutputContent
...
class SessionNextEventHandlerTest {
    ...
    fun `handle returns true for SessionNext event`() {
        val event = dev.leonardo.octether.domain.model.SseEvent.SessionNext(
            SessionNextEvent.AgentSwitched(sessionId = "s1", agent = "code")
        )
    ...
    fun `handle returns false for non-SessionNext event`() {
        val event = dev.leonardo.octether.domain.model.SseEvent.ServerHeartbeat
