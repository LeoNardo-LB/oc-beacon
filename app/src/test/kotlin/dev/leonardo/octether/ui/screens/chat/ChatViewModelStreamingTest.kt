...
import dev.leonardo.octether.domain.repository.ToolSnapshotCache
...
import dev.leonardo.octether.service.SessionFocusHolder
import dev.leonardo.octether.service.AppNotificationManager
...
import dev.leonardo.octether.ui.screens.sessions.SessionScrollSignal
import dev.leonardo.octether.ui.screens.chat.tools.DefaultToolCardResolver
import io.mockk.*
...
class ChatViewModelStreamingTest {
    ...
    private val pendingPromptRepository = mockk<dev.leonardo.octether.data.repository.PendingPromptRepository>(relaxed = true)

    private val messagesFlow = MutableStateFlow<List<Message>>(emptyList())
    private val partsFlow = MutableStateFlow<Map<String, List<dev.leonardo.octether.domain.model.Part>>>(emptyMap())
    ...
    fun setup() {
        ...
        chatRepository = mockk<ChatRepository>(relaxed = true).also {
            ...
            every { it.getParts(any()) } answers {
                ...
                partsFlow.map { map: Map<String, List<dev.leonardo.octether.domain.model.Part>> ->
                    map[sid] ?: emptyList()
                }
            }
            ...
            every { it.setMessages(any(), any()) } answers {
                val sid = firstArg<String>()
                val msgs = secondArg<List<dev.leonardo.octether.domain.model.MessageWithParts>>()
    ...
    private fun stubUserMessage(id: String = "msg-1") {
        ...
        val textPart = dev.leonardo.octether.domain.model.Part.Text(
            id = "$id-text",
            sessionId = "test-session",
            messageId = id,
            text = "hello"
        ...
        coEvery { manageSessionUseCase.listMessages(any(), any(), any()) } returns listOf(
            dev.leonardo.octether.domain.model.MessageWithParts(info = userMsg, parts = listOf(textPart))
        )
    }
