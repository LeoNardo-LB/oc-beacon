package dev.leonardo.octether.ui.screens.viewer
...
import dev.leonardo.octether.domain.usecase.GetFileContentUseCase
import dev.leonardo.octether.domain.usecase.GetFileDiffUseCase
import dev.leonardo.octether.domain.usecase.SubmitAnnotationsUseCase
...
class FileViewerViewModelTest {
    ...
    private val sampleDiffs = listOf(
        VcsFileDiff(
            ...
            additions = 4,
            deletions = 1,
            status = dev.leonardo.octether.domain.model.VcsStatus.MODIFIED
        )
    )
    ...
    fun `loadLive paginates content - initial visibleLineCount is 500 for large files`() = runTest {
        val largeContent = buildString {
            append("package dev.leonardo.octether\n\n")
    ...
    fun `init with json file sets fileType JSON`() = runTest {
        coEvery { getFileContent(any(), any(), any()) } returns Result.success(
            dev.leonardo.octether.domain.model.FileContent(
                path = "config.json",
                type = ContentType.TEXT,
                content = "{\"key\":\"value\"}"
            )
