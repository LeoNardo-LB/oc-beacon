...
import dev.leonardo.ocremoteplus.service.AppNotificationManager
...
import dev.leonardo.ocremoteplus.ui.screens.sessions.SessionScrollSignal
import io.mockk.*
...
class ChatViewModelSendTest {
    ...
    private fun createTestSession() = dev.leonardo.ocremoteplus.domain.model.Session(
        id = "test-session",
        title = "Test Session",
        directory = "/test",
        time = dev.leonardo.ocremoteplus.domain.model.Session.Time(created = 1000L, updated = 2000L)
    )
    ...
    private fun createViewModel(): ChatViewModel {
        ...
        return ChatViewModel(
            ...
            toolCardResolver = dev.leonardo.ocremoteplus.ui.screens.chat.tools.DefaultToolCardResolver(),
            chatRepository = mockk<ChatRepository>(relaxed = true).also {
                every { it.getAllPartsMap() } returns kotlinx.coroutines.flow.MutableStateFlow(emptyMap<String, List<dev.leonardo.ocremoteplus.domain.model.Part>>())
            },
