...
class ChatViewModelStreamingTest {
    ...
    fun setup() {
        ...
        chatRepository = mockk<ChatRepository>(relaxed = true).also {
            ...
            every { it.getParts(any()) } answers {
                ...
                partsFlow.map { map: Map<String, List<dev.leonardo.ocremoteplus.domain.model.Part>> ->
                    map[sid] ?: emptyList()
                }
            }
            ...
            every { it.setMessages(any(), any()) } answers {
                val sid = firstArg<String>()
                val msgs = secondArg<List<dev.leonardo.ocremoteplus.domain.model.MessageWithParts>>()
    ...
    private fun stubUserMessage(id: String = "msg-1") {
        ...
        val textPart = dev.leonardo.ocremoteplus.domain.model.Part.Text(
            id = "$id-text",
            sessionId = "test-session",
            messageId = id,
            text = "hello"
        ...
        coEvery { manageSessionUseCase.listMessages(any(), any(), any()) } returns listOf(
            dev.leonardo.ocremoteplus.domain.model.MessageWithParts(info = userMsg, parts = listOf(textPart))
        )
    }
