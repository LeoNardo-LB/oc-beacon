...
class MessagePaginationDelegateTest {
    ...
    @Test
    fun `loadOlderMessages doubles limit and sets hasOlderMessages by boundary`() = runTest {
        val useCase = mockk<ManageSessionUseCase> {
            coEvery { listMessages("srv", "sid-1", 60) } returns mkMessages(60)
        }
        ...
        assertEquals(60, delegate.currentLimitValue)
        assertTrue(delegate.hasOlderMessages.value)
        assertFalse(delegate.isLoadingOlder.value)
        coVerify(exactly = 1) { useCase.listMessages("srv", "sid-1", 60) }
    ...
    fun `loadOlderMessages sets hasOlderMessages false when fewer than limit`() = runTest {
        val useCase = mockk<ManageSessionUseCase> {
            coEvery { listMessages("srv", "sid-1", 60) } returns mkMessages(40)
        }
        val delegate = MessagePaginationDelegate(
            manageSessionUseCase = useCase,
            chatRepository = mockk(relaxed = true),
    ...
    @Test
    fun `loadOlderMessages halves limit back on exception`() = runTest {
        val useCase = mockk<ManageSessionUseCase> {
            coEvery { listMessages("srv", "sid-1", 60) } throws RuntimeException("net err")
        }
        val delegate = MessagePaginationDelegate(
            manageSessionUseCase = useCase,
            chatRepository = mockk(relaxed = true),
    ...
    fun `loadMessagesForSession applies settings initialMessageCount and sets hasOlderMessages`() = runTest {
        val useCase = mockk<ManageSessionUseCase> {
            coEvery { listMessages("srv", "sid-1", 50) } returns mkMessages(50)
        }
        val repo = mockk<ChatRepository>(relaxed = true)
    ...
    fun `loadMessagesForSession swallows exception without throwing`() = runTest {
        val useCase = mockk<ManageSessionUseCase> {
            coEvery { listMessages("srv", "sid-1", 30) } throws RuntimeException("boom")
        }
        val settings = mockk<SettingsRepository> {
