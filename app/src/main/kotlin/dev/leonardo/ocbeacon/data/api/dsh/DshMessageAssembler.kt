 ...
 * - MessagePartUpdated → 挂到对应 messageId 的 part 列表；**同 part id 多次更新
 *   原位合并**（#349：DSH wire 单卡恒多事件——tool/call Pending + tool/result
 *   Completed、code-dispatch Running/Completed/信封补写——原「按到达序 append」
 *   让历史页每卡携带全部中间态副本（×N 角标 + 陈旧首份胜出）。合并语义与
 *   dispatcher 的 MessageMergeEngine.mergePart 完全一致——历史重放与实况同路径）；
        ...
        for (event in events) {
            when (event) {
                ...
                is SseEvent.MessagePartUpdated -> {
                    val list = partsByMessage.getOrPut(event.part.messageId) { mutableListOf() }
                    val idx = list.indexOfFirst { it.id == event.part.id }
                    if (idx >= 0) list[idx] = MessageMergeEngine.mergePart(list[idx], event.part)
                    else list.add(event.part)
                }
