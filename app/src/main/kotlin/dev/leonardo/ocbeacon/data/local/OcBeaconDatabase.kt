...
/**
 * 应用本地数据库：消息缓存 + 诊断日志 + 冷存桶（v2）+ 堆积消息（v4）+ 会话同步状态（v5）。
 * 版本 1 建三表；v2 新增 archive_buckets；后续升级用 Migration 对象（禁止 DROP 重建）。
 */
@Database(
    entities = [CachedMessageEntity::class, CachedPartEntity::class, LogEntity::class, ArchiveBucketEntity::class, PendingMessageEntity::class, SessionSyncEntity::class],
    version = 5,
    exportSchema = false,
)
