...
- **修正（2026-08-07 真机根因实证）**：`clearForServer`/`clearAll` 是**连接状态清理**（stopConnection/stopAllConnections 调用）——临时断连/切换服务器**不得**移除 maxCompleted 或覆盖持久化（否则红点数据随连接停止丢失）；**仅 `SessionDeleted` 事件（会话真删）移除并同步落盘**
