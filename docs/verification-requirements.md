...
> 这类短暂/动态/主观现象，自动化测试与截图无法可靠捕捉。此类验证**必须由用户完成**，
> 自动化通过 ≠ 时间性现象正确。

**适用场景（涉及任一即必须走人工验证，Agent 不得默认自动化已覆盖）：**
- 时间性现象：加载过渡、蒙版揭/盖、闪烁、布局跳动、滚动抖动
- 动态数值：实时计时/计数、进度刷新、流式输出节奏
- 主观体验：视觉流畅度、过渡自然度、交互反馈感
...
**与自动化的分工（不互为替代）：**
- 自动化（单测/Maestro/截图/androidTest）：覆盖逻辑正确性、静态布局、可断言行为
...
- [ ] **V9**: 涉及**时间性/主观 UI 现象**（闪烁/动画/计时/布局跳动）时：提供人工验证清单（维度 5）+ 用户反馈记录；无此现象的纯逻辑/静态 UI 变更可跳过并注明理由
