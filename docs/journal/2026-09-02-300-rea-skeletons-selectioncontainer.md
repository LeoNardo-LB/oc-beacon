...
- **2026-09-03 用户报告「滚动中新气泡进入视口卡一下」（dev debug）**：
  stable release 0.3.0(40) 本地构建原位升级（release.jks 签名匹配）实测
  **基本无流畅度问题**——归因 debug 插桩（atrace/ScrollDiag 诊断）+ JIT
  无 R8 放大首组合成本，不立卡；R8 冒烟（启动/连接/思考卡渲染/FATAL 0）
  顺带验证混淆不破坏 Part 序列化链。
