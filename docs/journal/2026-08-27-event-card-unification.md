...
- **二阶修复（用户追问「是否根因」后升级，2026-08-29 04:00）**：①**机制级根因**——ScrollSpeedPrefetchStrategy 覆写 `prefetchScheduler` 为 no-op（调度即丢弃）→ pausable 预组合物理不可达 → 崩溃栈不可达；代价=放弃预组合平滑（分档窗口停用），实测关后 gfxinfo：fling 长文区 812 帧 jank 1.35%（p50 6ms/p90 13ms/p99 65ms=大 chunk 即时组合），流畅达标；恢复路径=删除该 override。②**FAB 水平贴边**——用户裁决「双 FAB 均无边距贴边」：⬇ 的 `start=16dp`（镜像菜单已消失的内部横距，与垂直漂移同族遗留）移除；装机 dump 实证 ⬇ [0,2160,144,2304]、菜单 [1056,2160,1200,2304]——左右 margin 双 0、基线一致。加固后再混沌 32 组手势 0 FATAL。
