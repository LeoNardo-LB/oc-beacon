...
### T2 滚动回归升级为立即上报项——并入 #245

新构建时长阶梯全档失败：120/300/500/800/1500ms ×3 同起点竖滑 15 次，画面 md5 与基线一致（帧差=0）；对照实验因消息区起点已全档不滚失去意义。**与上一轮「阵发性」观测合并定性：#245 升级——注入手势在新构建上全档失效、旧构建部分失效，真实人手影响面未知但滚动交互受损直接拖累一切人工复验效率**（本轮 T1 补测、B 会话翻找耗时极长均为其果）。诊断优先级高于残余视觉项；建议下一批次独立诊断（嫌疑域不变：isAtBottom/autoScroll/延迟揭示钳制族）。
