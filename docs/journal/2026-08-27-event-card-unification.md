...
## 二十六轮：V6 反馈修复——滚动中即时配对揭示+应用确认门（2026-08-28 23:00–23:20）

- **V6 首轮反馈定性**：①「滑完才展开/FAB 先停近底再跳」= 上轮为堵崩溃加的 expand 家族 holdReveal + 排空泵滚动闸门的**过度防御**——通道②对 drag 无断言，「滚动中注入=崩溃放大器」前提已消失，守卫该拆；②「闪烁跳几下」= 注入入队后同帧重测插队（拖动 forceRemeasure）误判增量已应用 → 揭示先于位移。用户「两个状态两个高度独立计算」直觉方向正确（原子切换），但高度只能测出不能算出——等效正解 = 滚动中即时配对 + 应用确认门。
...
- **二阶修复（用户追问「是否根因」后升级，2026-08-29 04:00）**：①机制级根因经 subagent 深研后**换用官方 flag 方案**（见三阶）；②**FAB 水平贴边**——用户裁决「双 FAB 均无边距贴边」：⬇ 的 `start=16dp`（镜像菜单已消失的内部横距，与垂直漂移同族遗留）移除；装机 dump 实证 ⬇ [0,2160,144,2304]、菜单 [1056,2160,1200,2304]——左右 margin 双 0、基线一致。
- **三阶（fling 顿挫/停住 + 崩溃最终形态，2026-08-29 04:30–05:00，/diagnosing-bugs 全流程）**：
  - **反馈环**：SafeFling 全出口带内速日志（zero-consume/decay/cancel/abort 四出口分类）+ 脚本化 fling（单向/双向/高速三族）+ gfxinfo 帧统计 + atrace 尝试（未激活 compose tracing，弃）。
  - **测量矩阵**：fast fling（60ms 甩）无 prefetch=p95 65/p99 129ms、legacy 75%（帧饥饿冻结=「突然停住+卡卡的」）；prefetch ON=p90 53/legacy 100%（预组合与渲染抢主线程=持续卡顿，且 pausable FATAL 暴露）；窗口 6→2 无改善；中速滚动两区（长文/卡区）全绿（jank 0.00-0.23%）。SafeFling 零消费出口全部发生在 v≈59px/s 自然衰减尾（合法），mid-list 冻结窗=0——「停住」非 fling 被杀，是 65-129ms 冻帧的观感。
  - **崩溃根因定音（subagent 三重证据）**：foundation 1.11.0 起静默重开 `ComposeFoundationFlags.isPausableCompositionInPrefetchEnabled`（1.10.6 曾官方默认关闭「address stability concerns」，I73960）→ pausable 预组合 × 非局部 return（Box inline lambda 内 `return@itemsIndexed` 跨组边界）→ GapComposer 组栈失衡（peek2 AIOOBE）。1.12.0 prefetch 相关源码与 1.11.2 逐字节 diff=0、无修复条目——升级路线证伪。
  - **最终修复（三件套，替代 no-op 方案）**：①OpenCodeApp.onCreate 最早置 `isPausableCompositionInPrefetchEnabled=false`（官方 1.10.6 同款止血）→ prefetch execute 走 performFullComposition 旧路径，**预组合完整保留 + pausable 机制整体绕开**；②R8 `-assumevalues` 固化（proguard-rules.pro）；③ScrollSpeedPrefetchStrategy 撤销 no-op 覆写（默认调度器+速度分档窗口原样）。
