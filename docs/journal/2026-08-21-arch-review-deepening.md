...
> 状态：进行中（P0 五候选 + P1 三项代码完成；批次末人工统一验收清单见文末「维度 5 验收清单」节）
...
| 验证项 | 证据 |
|--------|------|
完整回环 | 终端打字 → WebSocket → bash 执行 → 宿主机文件创建（`22.marker`）+ 回显渲染 |
vim 插入打字（原 bug①） | `hellovt` 上屏 + `-- 插入 --` 指示 |
vim ESC（原 bug②） | ESC 后回普通模式，文本保留 |
less 分页 | 彩色列表 + `:` 提示符 + `q` 退出回 shell |
键盘条 CTRL 锁存 | **回归**：字面 `c` ≠ ^C → 修复后 CTRL+c 中断 sleep 30（`^C` + 提示符回归，7e7f02e7） |
多 tab 创建 | 抽屉内 3 个标签并存 |
字号管线 | setTextSize 39px（13sp×3.0 density）日志实证 |
数据层 | JVM 单测 ×2（VT 序列 → emulator 缓冲；warmup→resize→feed 生产序列镜像）+ 全量套件绿 |

### 回归根因（7e7f02e7）
CTRL/ALT 锁存原由已删除的 sendTerminalChunk IME 路径消费；换件后 IME 直通 Termux view，修饰键走 `readControlKey()/readAltKey()`——只接了音量键状态。修复：锁存状态接入 read* 接口 + `onCodePoint` 消费清除（粘滞一键语义，对齐 Termux ExtraKeys）。

### adb 伪影边界（非 app bug，已记录）
- `type.sh` 丢标点（`>` `;` `/` `:`）→ shell 命令组合不可靠；纯字母序列可靠。真机手指输入无此问题。
- 视觉模型偶发误读（状态栏图标→「标签」）→ 关键判定用像素探针/OCR/宿主机文件实证。

### 维度 5 待用户手感（自动化不可覆盖）
双指缩放手感 · 长按选择复制 · 中文 IME 输入 · 抽屉边缘手势 · tab 切换点击

## 验收问题① 修复记录：后台通知未弹（2026-08-21 闭环）

### 根因（真机取证链，非应用投递故障）

取证路径：复现（API 发消息→后台）→ 通知**实际投递成功**（dumpsys：id=2129724473, channel=opencode_tasks, importance=4, `not_intercepted`）→ 但通知历史**零 buzz/beep** → 亮屏逐帧截图无横幅 → 打开渠道设置页实证：

| MIUI 渠道开关（opencode_tasks） | 修复前 | 说明 |
|------|------|------|
| 允许通知 | ✅ on | 所以抽屉里有——「未弹」实为「无横幅无声音」 |
| 悬浮通知 | ❌ off | **横幅不弹的直接原因** |
| 声音 | ❌ 无 | 无铃声 |
| 振动 | ❌ off | 无震动 |
| 锁屏显示 | ❌ 完全不显示 | 息屏收不到 |

**根因定性**：MIUI/HyperOS 对旁装载（side-load）应用的渠道默认关闭悬浮/声音/振动，且该策略**在标准 `NotificationChannel` API 中不可见**——渠道的 importance=4、mSound 有值，一切看起来健康，但 heads-up 永不显示。`#175` 抑制合并、通知路由、渠道创建全部无辜（复现日志 `Session idle -> Response ready` → 投递 → 抽屉可见，链路完好）。

### 修复（双层）

1. **设备侧**（当场）：渠道页开悬浮+振动+系统铃声，重跑后台通知 → 横幅复验通过（帧 8 清晰拍到「就绪·验收测试会话AB」横幅）。附注：设备当时处于静音模式（选铃声时提示「您当前为静音状态」），用户听不到声音还叠了这一层。
2. **代码侧根因**（`ac338b49`）：让不可见的系统配置可见、可行动——设置 > 通知新增两行：
   - **发送测试通知**：走真实任务渠道端到端投递（`AppNotificationManager.sendSelfTestNotification`，固定 id 防堆积，幂等建渠道），用户确认横幅/声音感知；
   - **系统通知设置**：`ACTION_APP_NOTIFICATION_SETTINGS` deep-link 直达渠道页，用户一键开悬浮。
   - i18n 8 键 × 15 语言，检查脚本 664 keys 全绿。

### 真机验证（新构建 pm install 后）

- 设置 > 通知 > 发送测试通知 → **横幅弹出**：「通知测试 / 如果你看到这条通知，说明通知通道工作正常。」✓
...
- 测试 flake 残余：ContextTokensTest 时序脆弱——**已于 #152 批次根因修复（1fefdb70）**


## 维度 5 批次末人工统一验收清单（2026-08-21，六卡一次过）

> 前置：真机已装最新 devDebug（versionCode 时间戳最新）。全部约 5-8 分钟。
> 通过标准：每项无回归即过；发现问题记卡片编号+现象，我修完再复验。

### A. #170 连接生命周期（~1.5 分钟）
1. Home 页「断开连接」→ 状态切换干净（无卡"连接中"、通知消失）
2. 重新「连接」→ 正常恢复，会话列表可刷
3. 开飞行模式 8s → 关 → 等 10-15s → 消息列表能正常加载（SSE 自愈）

### B. #171 未读红点（~1.5 分钟）
4. 会话列表：有红点的会话正常显示（本次会话不红——你正在看）
5. 点进任一红点会话 → 返回 → 红点消失
6. 杀进程重开 → 消失的保持消失
7. 更多菜单「一键已读」→ 全部红点消失；杀进程重开 → 保持 0

### C. #174 FSM 状态观感（~1 分钟）
8. 进会话发一条消息 → busy 状态正常（计时/流式输出/完成后转 idle）
9. 无"永久转圈"或状态错乱

### D. #175 清理回归（~1 分钟）
10. 消息列表滚动正常（主会话）
11. 有 subagent 的会话：进入子会话正常、无快速定位弹出
12. 后台收到消息 → 系统通知正常弹出（抑制逻辑合并后行为不变）

### E. #172 分页 + #173 簇重组（对话全流程，~2 分钟）
13. 长会话：滚到顶触发"加载更早" → 正常拼接无跳动
14. 快速定位 → 选目标跳转 → 位置正确
15. 输入草稿 → 切走再回 → 草稿恢复
16. 模型选择/agent 切换正常（BottomBar 迁移面）
17. 终端模式进出正常（Terminal 迁出面）
