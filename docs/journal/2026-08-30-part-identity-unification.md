...
> 状态：待验证（三层修复完成 + 全量单测 + 真机 E2E 六轮全绿；待用户验收收卡）
> 关联：#266（本批次即其实施）· #265（竞态两层守卫的根因层收口）· #246（锚点稳定性受益方）
...
- 干净构建装机后同款 prompt（三段诗 ×4 + 俳句等）连发 6+ 轮：渲染尾段均 ×1、无残渣；重进会话（Room 种子路径）渲染完整
- Room 拉库逐字比对：本会话**全部 17 条助手消息 text 行与服务器 REST 逐字一致**（82/55/226/255/41/185/1788/2022/1473/2927/353/315/299…），历史脏行已全部对齐
- SSE 抓包全程无异常事件序

## V6 验证清单（用户验收项）

1. 日常会话发数条长回复（多段/表格类），完结后尾句渲染 ×1
2. 重进会话后同一消息仍完整、无尾句重复（Room 路径）
3. 思考块（折叠 reasoning）展开内容无尾部重复
