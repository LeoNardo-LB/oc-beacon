...
### 轮次 N：2026-08-21 03:15-03:22（V2 真机 houji · 127.0.0.1:4199 经 adb reverse · devDebug 含 88774278 全部修复）
...
| 打字（/tmp/type.sh 纯 keyevent，合规禁 input text）+ 发送 | ✅ | 输入框文本逐键正确；发送键出现 | type.sh 已按 runbook 重建 |
...
**附带教训（观测问题归属）**：① 视觉模型对暗色主题聊天屏的「内容被裁切」判断不可靠（两次误报），须以 uiautomator 坐标/FAB 缺席/空隙像素佐证；② grep `text=\"[^\"]*\"` 会匹配空属性容器节点，统计文本节点须用 `text=\"[^\"][^\"]*\"`（曾险些误判 a11y 退化再现）。本轮全程 a11y 健康（含流式期间）。
