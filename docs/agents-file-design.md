...
| 级别 | 大写关键字 | 含义 | 本仓库示例 |
|------|-----------|------|-----------|
| 🔴 必读 | **MUST** | 绝对要求。该场景下**必须先读再行动**，跳过会导致错误、违规或返工 | `docs/release-workflow.md`（发版前）、`docs/verification.md`（声称完成前）、`docs/chatscreen-editing-protocol.md`（编辑 ChatScreen.kt 前） |
