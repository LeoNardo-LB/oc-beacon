...
**待裁决开口项（结论，2026-09-12 裁决 / 2026-09-17 修订）**：
1. 模型名放顶部还是底部？→ **底部**（尾部统计栏；弹窗逐轮明细亦含）。
2. 用户消息是否也去气泡？→ **保留气泡**（2026-09-12 一度裁决「去底色」，
   2026-09-17 用户否决：用户消息保留原三段式气泡；扁平化只作用于**智能体正文**）。
   与两端参考实现一致（dsh/opencode 均保留用户气泡、仅助手平面）。
3. 「所有类型」边界：→ **助手正文**去气泡；**用户消息保留气泡**；
   卡片层（工具/事件/压缩/提问卡）保留容器。
