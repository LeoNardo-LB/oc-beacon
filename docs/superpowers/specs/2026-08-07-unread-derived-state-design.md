...
- **代码净减少**：删除 `_turnEndTime`、forceComplete 红点写入、onTurnEnded 红点接线、lastReplyTime 持久化
- 历史数据：**允许一次性清空重建**（用户放开兼容约束）；已读三件套（readTimes/allReadAt/baseline）值域从客户端 now → 服务器 completed
- 范围：红点体系（EventDispatcher / MessageEventHandler / SessionListStateBuilder / SessionListViewModel / SettingsDataStoreReadTimes / ChatViewModel markRead 链 / 相关测试）。**不触碰**：聊天页渲染、SSE 流式管线、SessionStateService FSM 状态机（仅读 statusFlow）、列表状态切片（#23 产物）
...
isUnread(session) =
    ...
    && maxCompleted[session] > max(readTimes[sid], 基线, 一键已读)   // 全部服务器域
```

其中 `maxCompleted[session]` = 该会话最后一条完成 assistant 消息的 completed（服务器时刻，实时派生）。

### 3.1 maxCompleted flow（替代 _turnEndTime / lastReplyTime）
...
internal fun isUnread(
    sessionId: String,
    maxCompleted: Map<String, Long>,   // 原 lastReplyTime 参数，来源改为 eventDispatcher
...
- status == null（状态未知）→ 按不 Idle 处理（保守不红点，REST 同步后修正——与现状 idle 丢失依赖 L3 兜底等价）
...
| 调用点 | 现状 | 改为 |
|--------|------|------|
| ChatViewModel 退出（markSessionRead） | 内部 `System.currentTimeMillis()` | 调用方传入最后 completed（ChatViewModel 持有消息列表，`filterIsInstance<Assistant>().mapNotNull{completed}.maxOrNull()`）；无则跳过更新 |
| SessionReadSignal.markRead（内存即时信号） | ChatViewModel 传客户端 now | 传同一服务器 completed |
| 一键已读 markAllSessionsRead | 内部 now → allReadAt | 传入**已知会话全局 max** = max(所有会话的 maxCompleted)（listSessions 全量加载，已知=全局；搜索过滤不影响已加载集合） |
| 基线 ensureUnreadBaseline | 内部 now | 传入已知会话全局 max（语义等价：启用时已存在的回复全部 ≤ 全局 max → 无红点；之后的回复 > 全局 max → 红点） |

- DataStore **结构不变**（readTimes/allReadAt/baseline 的 key 与层级），仅**值域变化**（服务器 completed 替代客户端 now）
- 懒加载自洽性：未加载会话的 maxCompleted ≤ 一键已读/基线时的全局 max → 加载后自动无红点（无需补写）

### 3.4 一次性迁移（允许，用户放开约束）

- 版本标记（DataStore key：`unread_state_v2_migrated`）
- 首次运行新版：**清空 readTimes / allReadAt / unreadBaseline 三件套**（值域变化，旧客户端 now 值无法与新服务器域比较）
- 基线在下次判定前自动重建（ensureUnreadBaseline 逻辑保留，写入全局 max）→ 历史回复全部 ≤ 基线 → 升级后无红点全亮，之后按新机制积累
- 幂等（标记存在则跳过）
...
| lastReplyTime 持久化 collector（`replyTimePersistScope`） | EventDispatcher.kt:204-206 |
| `SettingsDataStore.saveLastReplyTimes` 相关 | SettingsDataStore 系列 |
| isUnread 的 lastReplyTime 语义（换 maxCompleted + status） | SessionListStateBuilder.kt |
...
SessionStatus=idle / L3 REST 校验 → statusFlow 状态 Idle ────┤
                                                            │
已读标记（服务器 completed）：退出会话 / 一键已读(全局 max) / 基线(全局 max) ──→ DataStore（值域服务器）
                                                            │
isUnread:  status==Idle && maxCompleted > max(readTimes, baseline, allReadAt)   ← 纯函数，快照派生
```
...
1. **全服务器域**：红点判定链路（maxCompleted / 已读标记 / 基线 / 一键已读）无任何 `System.currentTimeMillis()` 参与比较——单测断言 + 代码审查
...
5. **一键已读/基线**：全局 max 写入；懒加载会话加载后自动无红点——单测
6. **迁移幂等**：首次运行清空三件套；再次运行不清——单测
...
| 一键已读时未加载会话（全局 max 偏低） | listSessions 全量加载（无分页），已知 = 全局；plan 阶段验证 listSessions 拉取语义 |
| maxCompleted 全量扫描性能 | 增量维护（MessageUpdated 只在该会话 max 增加时更新；REST 整批低频重算） |
| 迁移清空已读状态 | 用户放开约束；基线自动重建防历史全亮 |
| 服务器时钟回拨（NTP） | completed 变小可能漏报，概率极低（秒级回拨）；无客户端时钟依赖，无偏移估计误差 |

## 7. 验证链

编译 ✅ → 新单测（maxCompleted 增量 / isUnread+status / 已读记录规则 / 一键已读基线 / 迁移）✅ → 全量单测 ✅ → 构建安装 ✅ → 真机回归（maestro/regression-unread-chain-a/b.yaml + 手动长 turn + 重启恢复）✅ → 完成声明前加载 verification-before-completion
...
| 迁移 | 清空 lastReplyTime | 清空已读三件套（值域变化） |
